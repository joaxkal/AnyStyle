"""
Implementation based on https://github.com/matthias-wright/art-fid/tree/master
...
def compute_content_distance(
    path_to_stylized, path_to_content, batch_size, content_metric="lpips", device="cuda", num_workers=1, gray=False
):
    ...
    dataloader_stylized = torch.utils.data.DataLoader(
        dataset_stylized, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=num_workers
    )
    ...
    dataloader_content = torch.utils.data.DataLoader(
        dataset_content, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=num_workers
    )
...
def compute_art_fid(
    ...
    if mode == "art_fid_inf":
        ...
        raise NotImplementedError("Style loss is not implemented!")
    else:
        raise NotImplementedError("Gram loss is not implemented!")

    print("Compute content distance...")
    cnt_value = compute_content_distance(
        path_to_stylized, path_to_content, batch_size, content_metric, device, num_workers
    )
    gray_cnt_value = compute_content_distance(
        path_to_stylized, path_to_content, batch_size, content_metric, device, num_workers, gray=True
    )
...
def main():
    ...
    parser.add_argument("--output_path", type=str, required=True, help="Path to results dir.")
    args = parser.parse_args()

    artfid, fid, lpips, lpips_gray = compute_art_fid(
        args.tar, args.sty, args.cnt, args.batch_size, args.device, args.mode, args.content_metric, args.num_workers
    )
    ...
    with open(args.output_path, "w") as f:
        f.write(f"ArtFID: {artfid}\n")
        f.write(f"FID: {fid}\n")
        f.write(f"LPIPS: {lpips}\n")
        f.write(f"LPIPS_gray: {lpips_gray}\n")
        f.write(f"CFSD: {cfsd}\n")
