...
histogram_block = RGBuvHistBlock(
    insz=max_input_size, h=histogram_size, intensity_scale=intensity_scale, method=method, device=device
)
...
for sty, tar in tqdm(zip(sty_list, tar_list), total=len(sty_list)):
    ...
    histogram_loss = (
        1
        / np.sqrt(2.0)
        * (torch.sqrt(torch.sum(torch.pow(torch.sqrt(target_hist) - torch.sqrt(input_hist), 2))))
        / input_hist.shape[0]
    )
    hist_list.append(histogram_loss)

cm_loss = (sum(hist_list) / len(hist_list)).item()

print("color matching loss:", cm_loss)

with open(opt.output_path, "w") as f:
    f.write(f"CM loss: {cm_loss}")
