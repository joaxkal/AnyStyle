...
class Metric(nn.Module):
    def __init__(self, metric_type="vgg"):
        ...
        if metric_type == "vgg":
            ...
            raise NotImplementedError("SSIM is not implemeneted!")
            ...
            raise NotImplementedError("MS-SSIM is not implemeneted!")
...
class PatchSimi(nn.Module):
    ...
    def forward(self, input, target):
        ...
        for idx in range(len(src_feats)):
            init_loss += F.kl_div(
                self.patch_simi_cnt(src_feats[idx]), self.patch_simi_out(target_feats[idx]), reduction="batchmean"
            )
        return init_loss
...
class GramLoss(nn.Module):
    def __init__(self, device=None):
        ...
        self.model = models.vgg19(pretrained=True).features.to(device).eval()
        self.layers = {"1": "conv1", "6": "conv2", "11": "conv3", "20": "conv4", "29": "conv5"}
        self.norm_mean = (0.485, 0.456, 0.406)
        self.norm_std = (0.229, 0.224, 0.225)
