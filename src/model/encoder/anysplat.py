...
from src.model.encoder.heads.vggt_dpt_gs_head_cross_attention import VGGT_DPT_GS_Head_Cross_Attention
from src.model.encoder.heads.vggt_dpt_gs_head_simple_injection import VGGT_DPT_GS_Head_Simple_Injection
from src.model.encoder.vggt.models.aggregator_injection import Aggregator_injection
from src.model.encoder.vggt.utils.geometry import (
    batchify_unproject_depth_map_to_point_map,
    unproject_depth_map_to_point_map,
)
from src.utils.text_adapter_utils import TextAdapter
...
class EncoderAnySplatCfg:
    ...
    style_geom_features: Literal["none", "no-pos", "colors-rotations"] = "none"
    cond_type: Literal["cross_attention_clip", "cross_attention_dino", "simple_injection_clip", "agg_injection_clip"] = "simple_injection_clip"
    text_adapter_return_identity: bool = True
    image_adapter_return_identity: bool = True
...
class EncoderAnySplat(Encoder[EncoderAnySplatCfg]):
    ...
    def __init__(self, cfg: EncoderAnySplatCfg) -> None:
        ...
        self.style_geom_features = cfg.style_geom_features
        self.cond_type = cfg.cond_type
        self.text_adapter_return_identity = cfg.text_adapter_return_identity
        self.image_adapter_return_identity = cfg.image_adapter_return_identity
        ...
        if self.use_style:
            ...
            self.text_adapter = TextAdapter(dim=512, 
                                            return_identity=self.text_adapter_return_identity, 
                                            return_identity_im=self.image_adapter_return_identity)
            
            #We only predict SH 0 for stylized gaussians, so override
            self.d_sh_old = self.gaussian_adapter.d_sh
            self.gaussian_adapter.cfg.sh_degree = 0
            self.raw_gs_dim_old = self.raw_gs_dim
            self.raw_gs_dim = 1 + self.gaussian_adapter.d_in

            ## WARNING!!!!!: although we predict SH=0 for stylized gaussians, please keep original output_dim!!
            #  its about pretrained wieghts! we can discuss it
            if self.cond_type =="cross_attention_clip":
                self.gaussian_param_head_style = VGGT_DPT_GS_Head_Cross_Attention(
                    dim_in=2048,
                    patch_size=head_params.patch_size,
                    output_dim=self.raw_gs_dim_old + 1, #HERE WARNING, output_dim must be old, for all sh
                    activation="norm_exp",
                    conf_activation="expp1",
                    features=head_params.feature_dim,
                    use_style=True,
            ...
            ## WARNING!!!!!: although we predict SH=0 for stylized gaussians, please keep original output_dim!!
            #  its about pretrained wieghts! we can discuss it
            elif self.cond_type =="cross_attention_dino":
                self.gaussian_param_head_style = VGGT_DPT_GS_Head_Cross_Attention(
                    dim_in=2048,
                    patch_size=head_params.patch_size,
                    output_dim=self.raw_gs_dim_old + 1, #HERE WARNING, output_dim must be old, for all sh
            ...
            ## WARNING!!!!!: although we predict SH=0 for stylized gaussians, please keep original output_dim!!
            #  its about pretrained wieghts! we can discuss it
            elif self.cond_type =="simple_injection_clip":
                self.gaussian_param_head_style = VGGT_DPT_GS_Head_Simple_Injection(
                    dim_in=2048,
                    patch_size=head_params.patch_size,
                    output_dim=self.raw_gs_dim_old + 1, #HERE WARNING, output_dim must be old, for all sh
                    activation="norm_exp",
            ...
            ## WARNING!!!!!: although we predict SH=0 for stylized gaussians, please keep original output_dim!!
            #  its about pretrained wieghts! we can discuss it
            elif self.cond_type =="agg_injection_clip":

                #add trainable aggregator copy (we removed some functionalities from it)
                self.aggregator_injection = Aggregator_injection( 
                    patch_size=head_params.patch_size, 
                    embed_dim=1024)
            ...
            #freeze NON style head
            for name, param in self.gaussian_param_head.named_parameters():
                param.requires_grad = False
    ...
    def forward(
        ...
        distill_infos = {}
        if self.distill:
            raise NotImplemented # TODO see  "package_only" and "attention only" forward modes for aggregtor. fix distill_aggregator
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
                    distill_aggregated_tokens_list, distill_patch_start_idx, _ = (
                        self.distill_aggregator(
        ...
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16): 
        # with torch.amp.autocast("cuda", enabled=False): # potentially fix_precision_jk
            
            ## TODO patch embedder is inside aggregator, not very elegant. However this way we can simply load pretrained anysplat weights.
            package = self.aggregator(
                images=image.to(torch.bfloat16), # potentially fix_precision_jk
                forward_mode = "package_only"
            )

            aggregated_tokens_list, patch_start_idx = self.aggregator(
                ready_package = package, forward_mode = "attention_only",
                intermediate_layer_idx=self.cfg.intermediate_layer_idx,
            )

            if self.use_style and self.cond_type=="agg_injection_clip":
                # aggregator_injection requires prepared package with patches and positions. For computation efficiency, no double patching
                aggregated_style_tokens_list, patch_start_idx = self.aggregator_injection(
                    style_dir=style_dir, ready_package = package, intermediate_layer_idx=self.cfg.intermediate_layer_idx,
                )
        ...
        if self.use_style:
            ...
            if self.cond_type=="agg_injection_clip":
                token_list = aggregated_style_tokens_list
            else:
                token_list = aggregated_tokens_list


            # dpt style gs_head input format
            out_stylized = self.gaussian_param_head_style(
                token_list,
                pts_all.flatten(0, 1).permute(0, 3, 1, 2),
                image,
                patch_start_idx=patch_start_idx,
                image_size=(h, w),
                style_dir=style_dir,
                patch_tokens=package[0][:,package[-1]:,:],
                frames_chunk_size=4
            )
            
            # get stylized colors
            colors = out_stylized[:, :, -(3*self.d_sh_old+1):-1, :, :] 
            B_color, C_color, D_color, H_color, W_color = colors.shape
            colors = colors.view(B_color, C_color, 3, self.d_sh_old, H_color, W_color)
            # For stylized image we only predict SH0, which is replaced in gaussian adapter already
            colors = colors[:, :, :, :self.gaussian_adapter.d_sh, :, : ]
            colors = colors.view(B_color, C_color, 3*self.gaussian_adapter.d_sh, H_color, W_color)
            
            # get original confidence
            confidence =  out[:, :, -1:, :, :] 

            match self.style_geom_features:
                case "none":
                    #stylize only colors
                    geom_features =  out[:, :, :-(3*self.d_sh_old+1), :, :]
                    out = torch.cat((geom_features, colors, confidence), dim=2)

                case "no-pos":
                    # stylize all params:colors, scales, rotations, opacities. BUT not positions! they are generated in other head.
                    # get geom features - stylized
                    geom_features =  out_stylized[:, :, :-(3*self.d_sh_old+1), :, :] 
                    
                    out = torch.cat((geom_features, colors, confidence), dim=2)
                
                case "colors-rotations":
                    # stylize only colors and rotations
                    # get geom features - stylized
                    opacities_scales =  out[:, :, :4, :, :] 
                    rotations =  out_stylized[:, :, 4:-(3*self.d_sh_old+1), :, :] 
                    
                    out = torch.cat((opacities_scales, rotations, colors, confidence), dim=2)

                case _:
                    raise ValueError(f"Invalid style_geom_features: {self.style_geom_features}")

        del aggregated_tokens_list, patch_start_idx, package
        if self.use_style:
            del out_stylized
            if self.cond_type=="agg_injection_clip":
                del aggregated_style_tokens_list
        torch.cuda.empty_cache()
