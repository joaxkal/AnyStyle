...
class VGGT_DPT_GS_Head_Simple_Injection(DPTHead):
    def __init__(self,
                 dim_in: int,
                 patch_size: int = 14,
                 output_dim: int = 83,
        ...
        if self.use_style:
            ...
            self.style_injector_direct = StyleInjector(
                out_channels=128,
            )
            ...
            self.style_injector_aggregator = nn.ModuleList([
                StyleInjector(
                out_channels=self.aggregator_embed_dim, hidden_dim =1024),
                StyleInjector(
                    out_channels=self.aggregator_embed_dim, hidden_dim =1024),
                StyleInjector(
                    out_channels=self.aggregator_embed_dim, hidden_dim =1024),
    ...
    def _forward_impl(self, encoder_tokens: List[torch.Tensor], imgs,
        ...
        if style_dir is not None and self.use_style:
            clip_vec = style_dir[3].to(self.style_injector_direct.fc[0].weight.dtype) # [B, 512]
        ...
        for layer_idx in self.intermediate_layer_idx:
            ...
            if self.use_style:
                style_to_inject = self.style_injector_aggregator[dpt_idx](clip_vec, target_size = x.shape[-2:])
                style_to_inject = repeat(style_to_inject, "b d w h-> (b v) d w h", v=S)
                x=x+style_to_inject
        ...
        if self.use_style:
            ...
            stylized_feat_direct = self.style_injector_direct(clip_vec, target_size = direct_img_feat.shape[-2:])

            stylized_feat_direct = repeat(stylized_feat_direct, "b d w h -> (b v) d w h", v=S)
...
class StyleInjector(nn.Module):
    ...
    def forward(self, clip_vec, target_size=None):
        ...
        projected_clip = projected_clip.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,target_size[0], target_size[1])
        projected_clip = self.zero_proj(projected_clip)
