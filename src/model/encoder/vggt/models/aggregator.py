...
class Aggregator(nn.Module):
    ...
    def forward(
        self,
        forward_mode: str = "full",  # options: "full", "package_only", "attention_only"
        images: Optional[torch.Tensor] =None,
        intermediate_layer_idx: Optional[List[int]] = None,
        ready_package: Optional[Tuple] = None
    ) -> Tuple[List[torch.Tensor], int]:
        """
        Args:
        """

        if forward_mode=="full" or forward_mode=="package_only":
            ...
            _, P, C = tokens.shape
        elif forward_mode=="attention_only":
            tokens, B, S, P, C, pos, patch_start_idx = ready_package

        if forward_mode=="package_only":
            return (tokens, B, S, P, C, pos, self.patch_start_idx)
