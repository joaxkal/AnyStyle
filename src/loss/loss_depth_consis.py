...
class LossDepthConsis(Loss[LossDepthConsisCfg, LossDepthConsisCfgWrapper]):
    ...
    def forward(
        ...
        distill_infos = depth_dict["distill_infos"]
        if "conf_mask" in distill_infos:
            valid_mask = distill_infos["conf_mask"]
        else:
            valid_mask = torch.ones(gt_rgb.permute(0,1,3,4,2).shape[:-1], dtype=torch.bool, device=gt_rgb.device)
