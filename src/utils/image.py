...
def prepare_image_for_dino_patches(im):

        # Crop so H, W are multiples of 14 (ViT patch size)
        _,_, H, W = im.shape
        patch_h, patch_w = 14, 14
        H_target = (H // patch_h) * patch_h
        W_target = (W // patch_w) * patch_w

        # Center crop
        top = (H - H_target) // 2
        left = (W - W_target) // 2
        im = tvf.functional.crop(im, top, left, H_target, W_target)
