...
def save_interpolated_video(
    pred_extrinsics, pred_intrinsics, b, h, w, gaussians, save_path, decoder_func, t=10, suffix=""
):
    ...
    # Save depth video
    save_video(depth_colored, os.path.join(save_path, f"depth{suffix}.mp4"))
    # Save video
    save_video(video, os.path.join(save_path, f"rgb{suffix}.mp4"))

    return os.path.join(save_path, f"rgb{suffix}.mp4"), os.path.join(save_path, f"depth{suffix}.mp4")
